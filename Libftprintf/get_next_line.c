...
/*   Updated: 2021/10/30 10:21:35 by hadufer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	c_lst = NULL;
	if (!get_next_line_util(&lst, &c_lst, fd, &buff[0]))
		return (NULL);
	...
	if (!buff[0])
		return (NULL);
	while (!ft_strchr(c_lst->buf, '\n') && read(fd, buff[0], BUFFER_SIZE) > 0)
	{
		buff[1] = c_lst->buf;
		c_lst->buf = ft_strjoin(c_lst->buf, buff[0]);
		free(buff[1]);
	}
	free(buff[0]);
	if (!ft_strchr(c_lst->buf, '\n'))
		return (c_lst->buf);
	return (ft_strndup(c_lst->buf, \
	ft_strlen(c_lst->buf) - ft_strlen(ft_strchr(c_lst->buf, '\n') + 1)));
}
